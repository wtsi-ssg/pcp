...
The lustre client currently only ships a static library (liblustreapi.a).
You can generate the dynamic library by doing the following:
...
try:
    ...
    print "***"
    print """ERROR: lustre.py requires liblustreapi.so. See the top of the lustre.py module
for instructions on how to obtain the library."""
    print "***"
    raise 
...
lustre.llapi_file_open.argtypes=[ctypes.c_char_p,ctypes.c_int,ctypes.c_int,ctypes.c_ulong,ctypes.c_int,ctypes.c_int,ctypes.c_int]
...
class stripeObj:
    ...
    Attributes:
      ...
      ostobjects[]: List of lov_user_ost_data_v1 structures as returned by the C API.
    """
    def __init__(self):
...
def setstripe(filename,stripeobj=None,stripesize=0,stripeoffset=-1,stripecount=1):
    ...
    fd=lustre.llapi_file_open(filename,flags,mode,stripesize,stripeoffset,stripecount,stripe_pattern)
    if fd<0:
        err=0-fd
        raise IOError(err,os.strerror(err))
    else:
